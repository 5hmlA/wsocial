...
import static jiang.wsocial.pic9.ViewState.sDuration;
...
public class ImageWatcher extends FrameLayout implements GestureDetector.OnGestureListener, ViewPager.OnPageChangeListener {
     ...
     *         被点击的ImageView的所在列表，加载图片时会提前展示列表中已经下载完成的thumb图片
     * @param urlList
     *         被加载的图片url列表，数量必须大于等于 imageGroupList.size。 且顺序应当和imageGroupList保持一致
     */
    public void show(ImageView i, List<ImageView> imageGroupList, final List<String> urlList){
        if(i == null || imageGroupList == null || urlList == null || imageGroupList.size()<1 || urlList
                .size()<imageGroupList.size()) {
            String info = "i["+i+"]";
    ...
    public void onUp(MotionEvent e){
        ...
        }else if(mTouchMode == TOUCH_MODE_SCALE_ROTATE || mTouchMode == TOUCH_MODE_LOCK) {
    ...
    public void onLongPress(MotionEvent e){
        if(mPictureLongPressListener != null) {
            mPictureLongPressListener
                    .onPictureLongPress(iSource, mUrlList.get(vPager.getCurrentItem()), vPager.getCurrentItem());
        }
    }
    ...
    private void handleScaleRotateGesture(MotionEvent e2){
        ...
        if(mFingersDistance == 0) {
            mFingersDistance = distance;
        }
        ...
        }else if(scaleResultX>MAX_SCALE) {
            scaleResultX = MAX_SCALE;
        }
        ...
        }else if(scaleResultY>MAX_SCALE) {
            scaleResultY = MAX_SCALE;
        }
        ...
        if(changedCenterXValue>maxTranslateX) {
            changedCenterXValue = maxTranslateX;
        }else if(changedCenterXValue<-maxTranslateX) {
            changedCenterXValue = -maxTranslateX;
        }
        ...
        if(changedCenterYValue>maxTranslateY) {
            changedCenterYValue = maxTranslateY;
        }else if(changedCenterYValue<-maxTranslateY) {
            changedCenterYValue = -maxTranslateY;
        }
    ...
    private void handleDragTouchResult(){
        ...
        if("horizontal".equals(imageOrientation)) {
            ...
            if(vsCurrent.translationY>translateYEdge) {
    ...
    public void onPageSelected(int position){
        ...
        if(position<mImageGroupList.size()) {
            ...
            if(iOrigin.getDrawable() != null) {
                //当前位置 对应 9宫格的图片 设置不可见
    ...
    class ImagePagerAdapter extends PagerAdapter {
        private final LayoutParams lpCenter = new LayoutParams(
        ...
        private boolean setDefaultDisplayConfigs(final ImageView imageView, final int pos, boolean hasPlayBeginAnimation){
            ...
            if(pos<mImageGroupList.size()) {
                ...
                //设置 vp当前的image view大小和 点击进入的图片一致
                imageView.getLayoutParams().width = originRef.getWidth();
                imageView.getLayoutParams().height = originRef.getHeight();

                ViewState.write(imageView, ViewState.STATE_ORIGIN).width(originRef.getWidth())
                        .height(originRef.getHeight());
                ...
                if(bmpMirror != null) {
                    ...
                    ViewState vsThumb = ViewState.write(imageView, ViewState.STATE_THUMB).width(bmpMirrorWidth)
                            .height(bmpMirrorHeight).translationX(( mWidth-bmpMirrorWidth )/2)
                            .translationY(( mHeight-bmpMirrorHeight )/2);

                    if(bmpMirror instanceof GifDrawable) {
                        originRef.destroyDrawingCache();
                        originRef.buildDrawingCache();
                        if(!originRef.getDrawingCache().isRecycled()) {
                            imageView.setImageBitmap(originRef.getDrawingCache());
                        }
                        ( (GifDrawable)bmpMirror ).stop();
                    }else {
            ...
            Glide.with(imageView.getContext()).load(mUrlList.get(pos)).apply(RequestOptions.errorOf(mErrorImageRes))
                    .into(new SimpleTarget<Drawable>() {
                        ...
                        public void onResourceReady(Drawable resource, Transition<? super Drawable> transition){
                            ...
                            ViewState vsDefault = ViewState.write(imageView, ViewState.STATE_DEFAULT)
                                    .width(sourceDefaultWidth).height(sourceDefaultHeight)
    ...
    private void animSourceViewStateTransform(ImageView view, final ViewState vsResult){
        ...
        animImageTransform = ViewState.restoreByAnim(view, vsResult.mTag).addListener(mAnimTransitionStateListener)
                .create();
            ...
            if(vsResult.mTag == ViewState.STATE_ORIGIN) {
                animImageTransform.addListener(new AnimatorListenerAdapter() {
                    ...
                    public void onAnimationEnd(Animator animation){
                        ...
                        if(iOrigin != null) {
                            iOrigin.setVisibility(View.VISIBLE);
                            if(iOrigin.getDrawable() instanceof GifDrawable) {
                                ( (GifDrawable)iOrigin.getDrawable() ).start();
                            }
                        }
...
//            animate().alpha(ViewState.read(view,vsResult.mTag).alpha).setDuration(300).start();
    ...
    private void animBackgroundTransform(final int colorResult){
        if(colorResult == mBackgroundColor) {
            return;
        }
        if(animBackground != null) {
            animBackground.cancel();
        ...
        animBackground = ValueAnimator.ofFloat(0, 1).setDuration(sDuration);
