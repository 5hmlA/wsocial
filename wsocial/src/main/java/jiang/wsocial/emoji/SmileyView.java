...
public class SmileyView extends LinearLayout implements ViewPager.OnPageChangeListener {
    ...
    private EmotionWrapper emotionInputHandler;
    ...
    private void init(Context context){
        ...
        setBackgroundColor(ContextCompat.getColor(context, R.color.wsocial_emoji_pannel_bg));
        COLOR_TAB = ContextCompat.getColor(context, R.color.wsocial_emoji_pannel_tabbg);
        COLOR_TAB_SEL = ContextCompat.getColor(context, R.color.wsocial_emoji_pannel_tab_selected_bg);
        ...
        gap.setBackgroundColor(ContextCompat.getColor(context, R.color.wsocial_emoji_pannel_divid_bottom));
        addView(gap);

        tabContainer = new LinearLayout(context);
        tabContainer.setOrientation(LinearLayout.HORIZONTAL);
        tabContainer.setGravity(Gravity.CENTER_VERTICAL);
        tabContainer.setBackgroundColor(ContextCompat.getColor(context, R.color.wsocial_emoji_pannel_bg));
...
//            SmileyDataSet setYwz = SmileyDataSet
//                    .getDataSet(context, "颜文字", SmileyDataSet.TYPE_TEXT, R.array.smiley_ywz);
//
//            smileys.add(setTieba);
//            smileys.add(setAcn);
//            smileys.add(GoogleEmoji.getEmojis());
//            smileys.add(setYwz);
            setSmileys(smileys = EmojiManager.smileys);
            viewPager.setAdapter(adapter);
        }
    }


    public void setInputView(EmotionWrapper handler){
    ...
    public void initTabs(){
        ...
        for(int i = 0; i<smileys.size(); i++) {
            ...
            final int finalI = i;
            if(i == 0) {
                itemView.setBackgroundColor(COLOR_TAB_SEL);
            }
    ...
    private class PageAdapter extends PagerAdapter {
        ...
        public Object instantiateItem(ViewGroup container, final int page){
            ...
            if(v == null) {
                ...
                v = new EmotionGridView(context, smileys.get(tabpos), COLOUM_COUNT, ROW_COUNT, index,
                        emotionInputHandler);
